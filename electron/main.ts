...
// Cookie Harvesting
async function harvestYouTubeCookies(): Promise<void> {
    try {
...
// Background Task
let lastSuccessfulCookieHarvest = Date.now();
setInterval(async () => {
    const now = Date.now();
    if (now - lastSuccessfulCookieHarvest < 4 * 60 * 60 * 1000) return;
    if (activeSearches.size > 0 || activeDownloads.size > 0) return;

    try {
        await harvestYouTubeCookies();
        lastSuccessfulCookieHarvest = Date.now();
    } catch (err) {
        console.warn('[YtCookies] Background harvest failed:', err);
    }
}, 10 * 60 * 1000);
    ...
    try {
      ...
      if (rangeHeader) {
        ...
        if (match) {
          ...
          return new Response(Readable.toWeb(fs.createReadStream(localPath, { start, end })) as any, {
            status: 206,
            headers: { ...commonHeaders, 'Content-Type': mimeTypes[ext] || 'audio/mp4', 'Content-Range': `bytes ${start}-${end}/${stat.size}`, 'Content-Length': String(end - start + 1) }
          });
        }
      }

      return new Response(Readable.toWeb(fs.createReadStream(localPath)) as any, {
        status: 200,
        headers: { ...commonHeaders, 'Content-Type': mimeTypes[ext] || 'audio/mp4', 'Content-Length': String(stat.size) }
      });

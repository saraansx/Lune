...
export function generateTOTP(
    ...
    const counterValue = Math.floor(epoch / stepSeconds);
    const counterBuffer = Buffer.alloc(8);
    counterBuffer.writeUInt32BE(0, 0); // Avoid larger than 32-bit since JS math
    counterBuffer.writeUInt32BE(counterValue, 4);

    const key = base32Decode(secret);

    // Provide the key parameter as Uint8Array
    ...
    const hash = Buffer.from(hmacResult);
